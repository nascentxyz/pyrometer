...
impl BuiltinRange {
	pub fn elem_from_dyn(&self, idx: NodeIdx, range_side: DynamicRangeSide, loc: Loc) -> BuiltinElem {
		match self {
        ...
        match builtin {
            ...
            Builtin::Int(size) => {
                if *size == 256 {
                    ...
                    let max: I256 = I256::from_raw(U256::from(1u8) << U256::from(size - 1)) - 1.into();
            ...
            Builtin::Bool => {
                Some(BuiltinRange::Bool(BoolRange::default()))
            }
            _ => None,
        }
    }
