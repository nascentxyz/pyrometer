...
impl ContextVarNode {
	...
	pub fn loc<'a>(&self, analyzer: &'a impl AnalyzerLike) -> Loc {
		self.underlying(analyzer).loc.expect("No loc for contextvar")
	}
...
impl ContextVar {
	...
	pub fn maybe_new_from_field(analyzer: &impl AnalyzerLike, loc: Loc, parent_var: &ContextVar, field: Field) -> Option<Self> {
	...
	pub fn maybe_new_from_func_param(analyzer: &impl AnalyzerLike, param: FunctionParam) -> Option<Self> {
	...
	pub fn maybe_new_from_func_ret(analyzer: &impl AnalyzerLike, ret: FunctionReturn) -> Option<Self> {
