...
impl SolcRange {
    ...
    pub fn update_deps(&mut self, node: ContextVarNode, ctx: ContextNode, analyzer: &impl GraphLike) {
        let deps = self.dependent_on();
        let mapping: BTreeMap<NodeIdx, NodeIdx> = deps
            .into_iter()
            ...
            .map(|dep| {
                ...
                if latest == node {
                    if let Some(prev) = latest.previous_version(analyzer) {
                        (dep.0.into(), prev.0.into())
                    } else {
                        (dep.0.into(), dep.0.into())
                    }
                } else {
                    (dep.0.into(), latest.0.into())
                }
            })
            .collect();
