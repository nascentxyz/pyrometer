...
impl SourceUnitPartNode {
    ...
    pub fn visible_funcs<'a>(&self, analyzer: &'a impl GraphBackend) -> Result<&'a Vec<FunctionNode>, GraphError> {
    	Ok(&self.underlying(analyzer)?.funcs)
    }

    pub fn visible_structs<'a>(&self, analyzer: &'a impl GraphBackend) -> Result<&'a Vec<StructNode>, GraphError> {
    	Ok(&self.underlying(analyzer)?.structs)
    }

    pub fn visible_constants<'a>(&self, analyzer: &'a impl GraphBackend) -> Result<&'a Vec<VarNode>, GraphError> {
    	Ok(&self.underlying(analyzer)?.constants)
    }

    pub fn visible_contracts<'a>(&self, analyzer: &'a impl GraphBackend) -> Result<&'a Vec<ContractNode>, GraphError> {
    	Ok(&self.underlying(analyzer)?.contracts)
    }

    pub fn add_func(&self, func: FunctionNode, analyzer: &mut impl GraphBackend) -> Result<(), GraphError> {
    	self.underlying_mut(analyzer)?.funcs.push(func);
    	Ok(())
    }

    pub fn add_struct(&self, strukt: StructNode, analyzer: &mut impl GraphBackend) -> Result<(), GraphError> {
    	self.underlying_mut(analyzer)?.structs.push(strukt);
    	Ok(())
    }

    pub fn add_contract(&self, contract: ContractNode, analyzer: &mut impl GraphBackend) -> Result<(), GraphError> {
    	self.underlying_mut(analyzer)?.contracts.push(contract);
    	Ok(())
    }

    pub fn add_constant(&self, constant: VarNode, analyzer: &mut impl GraphBackend) -> Result<(), GraphError> {
