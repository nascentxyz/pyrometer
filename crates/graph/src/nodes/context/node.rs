                ...
                inputs.iter().for_each(|input| {
                    ...
                    if let Some((param, replacement_input)) = mapping.get(&name) {
                        if let Some(param_ty) =
                            VarType::try_from_idx(analyzer, param.ty(analyzer).unwrap())
                        {
                ...
                if body_ctx.underlying(analyzer)?.child.is_some() {
                    ...
                    let ret: Vec<_> = body_ctx
                        ...
                        .map(|(i, (_, ret_node))| {
                            ...
                            let new_cvar =
                                ContextVarNode::from(analyzer.add_node(Node::ContextVar(new_var)));
                            analyzer.add_edge(
                                new_cvar,
                                *self,
                    ...
                    self.push_expr(ExprRet::Multi(ret), analyzer)?;
                    return Ok(true);
                }
            }
