...
use crate::range::range_string::RangeElemString;
...
impl Atomize for Elem<Concrete> {
    fn atoms_or_part(&self) -> AtomOrPart {
        match self {
            Elem::Concrete(_) | Elem::Dynamic(_) => AtomOrPart::Part(self.clone()),
            ...
            Elem::Expr(expr) => {
                ...
                match (expr.lhs.atoms_or_part(), expr.rhs.atoms_or_part()) {
                    (ref lp @ AtomOrPart::Part(ref l), ref rp @ AtomOrPart::Part(ref r)) => {
                        match (l, r) {
                            (Elem::Dynamic(Dynamic { .. }), Elem::Concrete(_))
                            | (Elem::Concrete(_), Elem::Dynamic(Dynamic { .. })) => {
                            ...
                            (Elem::Dynamic(Dynamic { .. }), Elem::Dynamic(Dynamic { .. })) => {
                            ...
                            (Elem::Expr(_), Elem::Dynamic(Dynamic { .. })) => {
                                todo!("here1");
                            }
                            (Elem::Dynamic(Dynamic { .. }), Elem::Expr(_)) => {
                                todo!("here2");
                            }
    ...
    fn atomize(&self) -> Option<SolverAtom> {
        ...
        match self {
            Dynamic(_) => None,     //{ println!("was dyn"); None},
