use crate::{
    ...
    variable::Variable, ExprErr, IntoExprErr, ExpressionParser
};
...
impl<T> PrePostIncDecrement for T where T: AnalyzerBackend<Expr = Expression, ExprErr = ExprErr> + Sized {}
/// Handles pre and post increment and decrement
pub trait PrePostIncDecrement: AnalyzerBackend<Expr = Expression, ExprErr = ExprErr> + Sized {
	/// Handle a preincrement
	fn pre_increment(&mut self, expr: &Expression, loc: Loc, ctx: ContextNode) -> Result<(), ExprErr> {
	...
	fn post_increment(&mut self, expr: &Expression, loc: Loc, ctx: ContextNode) -> Result<(), ExprErr> {
		self.parse_ctx_expr(expr, ctx)?;
        self.apply_to_edges(ctx, loc, &|analyzer, ctx, loc| {
	...
	fn pre_decrement(&mut self, expr: &Expression, loc: Loc, ctx: ContextNode) -> Result<(), ExprErr> {
		self.parse_ctx_expr(expr, ctx)?;
        self.apply_to_edges(ctx, loc, &|analyzer, ctx, loc| {
	...
	fn post_decrement(&mut self, expr: &Expression, loc: Loc, ctx: ContextNode) -> Result<(), ExprErr> {
		self.parse_ctx_expr(expr, ctx)?;
        self.apply_to_edges(ctx, loc, &|analyzer, ctx, loc| {
